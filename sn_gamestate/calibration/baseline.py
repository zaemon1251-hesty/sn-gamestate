...
class BaselineCalibration(ImageLevelModule):
    ...
    def process(self, batch: Any, detections: pd.DataFrame, metadatas: pd.DataFrame):
        ...
        for k, v in predictions.items():
            ...
            p1 = np.array(
                [
                    v[0]["x"] * self.resolution_width,
                    v[0]["y"] * self.resolution_height,
                    1.0,
                ]
            )
            p2 = np.array(
                [
                    v[1]["x"] * self.resolution_width,
                    v[1]["y"] * self.resolution_height,
                    1.0,
                ]
            )
        ...
        if len(line_matches) >= 4:
            target_pts = [
                field.point_dict[k][:2] for k in potential_3d_2d_matches.keys()
            ]
            ...
            if success:
                ...
                if success:
                    ...
                    for k, potential_matches in potential_3d_2d_matches.items():
                        ...
                        if (
                            0 < projected[0] < self.resolution_width
                            and 0 < projected[1] < self.resolution_height
                        ):
